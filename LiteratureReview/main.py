...
if __name__ == "__main__":
    """
    driver = webdriver.Chrome()
    c = CCDCCitingPaper("BERKEO", driver = driver, json_file = "LiteratureReview/citing.json")
    c.get_original_papers()
    c.citer(False)
    keyword_reviewer = KeyWordSearch("BERKEO", "output.txt", driver = driver)
    keyword_reviewer.search(c.citing_papers)
    driver.close()
    """
    """
    id = "ZZZDKE01"
    ...
    """
    """
    ids = ("POSGUZ", "DNBZFX01", "XATKEJ", "DHNAPH17", "MAVRAD", "CAPJIN", "LOCVEE", "DEDDEV")
    keyword_llm_pipeline(ids, "LiteratureReview/osc_tadf.txt")
    """