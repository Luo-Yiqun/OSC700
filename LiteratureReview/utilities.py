...
def parse_keywords(s: str) -> list[str]:
    return ast.literal_eval(re.search(r'\[[^[]+\]', s).group(0))
            ...
            for line in f:
                ...
                # elif "RAG is doing LLM literature extraction for" in line:
                    ...
                    if "Warning: The prompt has" in next_line and "characters, which is too long. Skipping. Please manually check this paper." in next_line:
                        tmp = int(re.search(r'\d+', next_line).group())
                        n_chars.append(tmp)
...
def json_to_pandas(sets: tuple[str] = ("kaiji", "sub1ev", "100-125ev", "125-150ev", "150-175ev", "175-200ev", "osc_tadf"), data_dir: str = None) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    if data_dir is None:
            ...
            for title, content in papers.items():
                ...
                if "llm_exp" in content:
                    for extraction in content["llm_exp"]:
                        ...
                        elif any([extraction.get(prop, None) for prop in ("density_of_state", "absorption_spectrum", "optical_gap", "optical_gap_source")]):
                ...
                if "llm_cal" in content:
                    for extraction in content["llm_calc"]:
                        if "Prompt too long" in (extraction.get("notes", '') or ''):
                            df_inaccessible.loc[len(df_inaccessible)] = {"Set": s, "ID": id, "Title": title, "Link": link, "Notes": extraction["notes"]}
                        elif any([extraction.get(prop, None) for prop in ("bse_gap", "bse_gap_source", "other_gw_bse_results")]):
                            for link in content.get("link", [None]):
                                df_calc.loc[len(df_calc)] = {
                                    ...
                                    "bse_gap": extraction.get("bse_gap"),
                                    "bse_gap_source": extraction.get("bse_gap_source"),
                                    "other_gw_bse_results": extraction.get("other_gw_bse_results"),
                                    "Link": link,
                                }
...
if __name__ == "__main__":
    """
    # results = parse_output("LiteratureReview/pah101_0_60.txt")[0]
    # results = combine_dicts(results, parse_output("LiteratureReview/pah101_60_80.txt")[0])
    # results = combine_dicts(results, parse_output("LiteratureReview/pah101_80_101.txt")[0])
    results = parse_output("LiteratureReview/osc_tadf.txt")[0]
    print(f"There are {len(results)} structures with results.")
    with open("LiteratureReview/osc_tadf.json", "w", encoding="utf-8") as f:
    ...
    with open("LiteratureReview/pah101_paper.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    comb_to_input = defaultdict(list)
    comb_to_output = defaultdict(list)
    for id, papers in data.items():
        for paper, values in papers.items():
            if paper != "finished":
                for result in values.get("llm_exp", []):
                    comb_to_input[(result["model"], result["rag"])].append(result["prompt_tokens"])
                    comb_to_output[(result["model"], result["rag"])].append(result["completion_tokens"])
    for comb, tokens in comb_to_input.items():
        print(comb, np.mean(tokens))
    for comb, tokens in comb_to_output.items():
        print(comb, np.mean(tokens))
    """
    """
    with open("LiteratureReview/citing.json", "r", encoding="utf-8") as f:
    ...
    """
    """
    df_inaccessible, df_exp, df_calc, df_devices = json_to_pandas(sets = ("pah101",))
    df_inaccessible.to_csv("LiteratureReview/pah101_inaccessible.csv", index=False)
    """
    """
